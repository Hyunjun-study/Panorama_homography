...
def extract_gradient_mask_region(mask):
    coords = cv.findNonZero((mask > 0).astype(np.uint8))
...
# 이미지 로드
img1 = cv.imread('morning_1.jpg')
img2 = cv.imread('nice_2.jpg')
img3 = cv.imread('final_3.jpg')
...
strong_weighted_mask3_2 = create_stronger_weighted_masks(overlap_mask2_3, direction='horizontal', reverse=True, strength=70.0, blur_size=(101, 101))
...
# 그래디언트 영역만 추출
cropped_mask2_1 = extract_gradient_mask_region(strong_weighted_mask2_1)
cropped_mask3_2 = extract_gradient_mask_region(strong_weighted_mask3_2)

highlighted_canvas = canvas.copy()
highlighted_canvas[strong_weighted_mask2_1 > 0] = [0, 255, 0]  # 겹치는 영역 1-2는 초록색으로 표시
highlighted_canvas[strong_weighted_mask3_2 > 0] = [255, 0, 0]  # 겹치는 영역 2-3은 빨간색으로 표시

# 시각화
plt.figure(figsize=(10, 5))
plt.imshow(cv.cvtColor(highlighted_canvas, cv.COLOR_BGR2RGB))
plt.title("Highlighted Overlapping Regions")
plt.axis('off')
plt.show()
...
plt.title("Cropped Mask 2->1")
plt.axis('off')

plt.subplot(1, 2, 2)
plt.imshow(strong_weighted_mask3_2, cmap='gray')
plt.title("Cropped Mask 3->2")
plt.axis('off')
plt.show()
...
plt.axis('off')
plt.title("OtherTime Final Image")
plt.show()
